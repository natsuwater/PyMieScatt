...
def Inversion(
    ...
    kMin=0.001,
    kMax=1,
    scatteringPrecision=0.010,
    absorptionPrecision=0.010,
    spaceSize=120,
    interp=2,
):
    ...
    scaSolutions = np.where(
        np.logical_and(
            Qsca * (1 - scatteringPrecision) < scaSpace,
            scaSpace < Qsca * (1 + scatteringPrecision),
        )
    )
    absSolutions = np.where(
        np.logical_and(
            Qabs * (1 - absorptionPrecision) < absSpace,
...
def Inversion_SD(
    ...
    kMin=0,
    kMax=1,
    scatteringPrecision=0.001,
    absorptionPrecision=0.001,
    spaceSize=40,
    interp=2,
):
    dp = coerceDType(dp)
    ...
    scaSolutions = np.where(
        np.logical_and(
            Bsca * (1 - scatteringPrecision) < scaSpace,
            scaSpace < Bsca * (1 + scatteringPrecision),
        )
    )
    absSolutions = np.where(
        np.logical_and(
            Babs * (1 - absorptionPrecision) < absSpace,
...
def ContourIntersection(
    ...
    if n is None:
        ...
        if type(n) in [list, tuple, np.ndarray]:
            ...
            scaChart = ax.vlines(
                n[0], kMin, kMax, linestyle="dashdot", linewidth=1.5, color="r"
            )
        else:
            scaChart = ax.vlines(
                n, kMin, kMax, linestyle="dashdot", linewidth=1.5, color="r"
            )

    if k is None:
        absChart = ax.contour(
            _n, _k, QabsList, absLevels, origin="lower", linewidths=1.5, colors=("blue")
        )
        ...
        if type(k) in [list, tuple, np.ndarray]:
            ...
            absChart = ax.hlines(
                k[0], nMin, nMax, linestyle="solid", linewidth=1.5, color="b"
            )
        else:
            absChart = ax.hlines(
                k, nMin, nMax, linestyle="solid", linewidth=1.5, color="b"
            )

    if Qback is not None:
        backChart = ax.contour(
            _n,
            _k,
            QbackList,
            backLevels,
    ...
    if n is None and k is None:
        ...
        nSolutionsToPlot, kSolutionsToPlot = (
            [x.real for x in solutionSet],
    ...
    if Qback is None:
        if incErrors:
            ...
            graphElements = {
                ...
                "Qabs": _c[1],  # contours
                "QscaErrFill": _c[2],
                "QscaErrOutline1": _c[3],
                "QscaErrOutline2": _c[4],
                "QabsErrFill": _c[5],
                "QabsErrOutline1": _c[6],
                "QabsErrOutline2": _c[7],
                "SolMark": _c[8],
                "SolFill": _c[9],  # the circly thingies at each solutions
                "CrosshairsH": _c[10:-10:2],
                "CrosshairsV": _c[11:-10:2],  # solution crosshairs
                "LeftSpine": _c[-10],
                "RightSpine": _c[-9],
                "BottomSpine": _c[-8],
                "TopSpine": _c[-7],  # spines
                "XAxis": _c[-6],
                "YAxis": _c[-5],
            }  # the axes
        else:
            ...
            graphElements = {
                ...
                "Qabs": _c[1],  # contours
                "SolFill": _c[2],
                "SolMark": _c[3],  # the circly thingies at each solutions
        ...
        if incErrors:
            ...
            graphElements = {
                ...
                "Qback": _c[2],  # contours
                "QscaErrFill": _c[4],
                "QscaErrOutline1": _c[5],
                "QscaErrOutline2": _c[6],
                "QabsErrFill": _c[7],
                "QabsErrOutline1": _c[8],
                "QabsErrOutline2": _c[9],
                "SolMark": _c[10],
                "SolFill": _c[11],  # the circly thingies at each solutions
                "CrosshairsH": _c[12:-10:2],
                "CrosshairsV": _c[13:-10:2],  # solution crosshairs
                "LeftSpine": _c[-10],
                "RightSpine": _c[-9],
                "BottomSpine": _c[-8],
                "TopSpine": _c[-7],  # spines
            ...
            graphElements = {
                ...
                "Qback": _c[2],  # contours
                "SolFill": _c[3],
                "SolMark": _c[4],  # the circly thingies at each solution
                "CrosshairsH": _c[5:-10:2],
                "CrosshairsV": _c[6:-10:2],  # solution crosshairs
                "LeftSpine": _c[-10],
                "RightSpine": _c[-9],
                "BottomSpine": _c[-8],
                "TopSpine": _c[-7],  # spines
...
def ContourIntersection_SD(
    ...
    if Bback is not None:
        Bback *= 1e6
    if (type(Babs) == np.float64 and Babs == 0.0) or (
        type(Babs) in [list, tuple, np.ndarray] and Babs[0] == 0
    ):
        k = 0.0
    if k == 0.0:
        kMin = -0.1
    ...
    if k is None:
        absChart = ax.contour(
            _n, _k, BabsList, absLevels, origin="lower", linewidths=1.5, colors=("blue")
        )
        if absError is not None:
    ...
    if Bback is None:
        if incErrors:
            ...
            graphElements = {
                ...
                "Labels": labels,
                "BscaErrFill": _c[2],
                "BscaErrOutline1": _c[3],
                "BscaErrOutline2": _c[4],
                "BabsErrFill": _c[5],
                "BabsErrOutline1": _c[6],
                "BabsErrOutline2": _c[7],
                "SolMark": _c[8],
                "SolFill": _c[9],  # the circly thingies at each solutions
                "CrosshairsH": _c[10:-10:2],
                "CrosshairsV": _c[11:-10:2],  # solution crosshairs
        ...
        if incErrors:
            ...
            graphElements = {
                ...
                "Labels": labels,
                "BscaErrFill": _c[4],
                "BscaErrOutline1": _c[5],
                "BscaErrOutline2": _c[6],
                "BabsErrFill": _c[7],
                "BabsErrOutline1": _c[8],
                "BabsErrOutline2": _c[9],
                "SolMark": _c[10],
                "SolFill": _c[11],  # the circly thingies at each solutions
                "CrosshairsH": _c[12:-10:2],
                "CrosshairsV": _c[13:-10:2],  # solution crosshairs
...
def SurveyIteration(Qsca, Qabs, wavelength, diameter, tolerance=0.0005):
    ...
    initial_m = Inversion(
        ...
        wavelength,
        diameter,
        scatteringPrecision=0.015,
        absorptionPrecision=0.015,
        spaceSize=85,
        interp=2,
    )
    print(len(initial_m))
...
def SurveyIteration_SD(Bsca, Babs, wavelength, dp, ndp, tolerance=0.0005, SMPS=True):
    ...
    ndp = coerceDType(ndp)
    error = lambda measured, calculated: np.abs((calculated - measured) / measured)
    initial_m = Inversion_SD(
        Bsca,
        Babs,
        wavelength,
        dp,
        ndp,
        scatteringPrecision=0.08,
        absorptionPrecision=0.08,
        spaceSize=40,
        interp=2,
    )
